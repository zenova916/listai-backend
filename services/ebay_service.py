...
async def publish_to_ebay(listing: dict, access_token_enc: str, sandbox: bool = False) -> dict:
    ...
    # Log full eBay response for debugging
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"[eBay] AddItem Ack: {ack}")
    logger.info(f"[eBay] AddItem ItemID: {item_id}")
    logger.info(f"[eBay] Full response: {r.text[:1000]}")

    if not item_id:
        # Extract warnings/errors for better error message
        warnings = root.findall(".//e:Error", namespaces=ns)
        msgs = [e.findtext("e:LongMessage", namespaces=ns) or "" for e in warnings]
        raise Exception(f"eBay returned no ItemID. Ack={ack}. Messages: {'; '.join(msgs) or 'No details'}")
